(function() {
  ...
  var Menu = Ace.Menu = {
    initialize: function (animator) {
      ...
      this.setupTracingArmOptions(this.$el.find('fieldset.tracing-arm-set'))
    },
    ...
    setupTracingArmOptions: function ($el) {
      this.hookOscillatingTracingDistance($el.find("#oscillating-tracing-distance"));
    },
    ...
    hookOscillatingTracingDistance: function ($el) {
      $el.prop("checked", Ace.RollingTracer.oscillatingTracingDistance);
      $el.change(function(e) {
        Ace.RollingTracer.oscillatingTracingDistance = $(e.currentTarget).is(":checked");
      })
    },
