(function() {
	...
	var RollingTracer = Ace.RollingTracer = function (options) {
		...
		this.rod = this.tracingPoint;
		this.spinAlive = false;
		this.hyperAlive = this.spinAlive;
		this._path = [];
	...
	RollingTracer.hyperPeriod = RollingTracer.spinPeriod;
	...
	RollingTracer.SPIN_PERIOD = RollingTracer.period() * (1);
	...
	var aliasStatic = function (oldName, newName) {
		Object.defineProperty(RollingTracer, oldName, {
			get: function () { return RollingTracer[newName]; },
			set: function (value) { RollingTracer[newName] = value; }
		});
	};

	aliasStatic("drawPlane", "drawRollingTracer");
	aliasStatic("PIXEL_DIM", "TRACE_PIXEL_SIZE");
	aliasStatic("periodMult", "centerPeriodMult");
	aliasStatic("hyperMult", "spinPeriodMult");
	aliasStatic("HYPER_PERIOD", "SPIN_PERIOD");
	aliasStatic("hyperDrift", "spinDrift");
	aliasStatic("ROD_MULT", "TRACING_DISTANCE_MULT");
	aliasStatic("ROD_LENGTH", "TRACING_DISTANCE");
	aliasStatic("ROD_WIDTH", "TRACING_ARM_WIDTH");
	aliasStatic("rodDrift", "tracingDistanceDrift");
	aliasStatic("rodRodToggle", "oscillatingTracingDistance");
	aliasStatic("sinStore", "centerSinStore");
	aliasStatic("cosStore", "centerCosStore");
	aliasStatic("hyperSinStore", "spinSinStore");
	aliasStatic("hyperCosStore", "spinCosStore");

	Object.defineProperty(RollingTracer, "steps", {
		get: function () { return Ace.Animator ? Ace.Animator.stepsPerFrame : 15; },
		set: function (value) {
			if (Ace.Animator) {
				Ace.Animator.stepsPerFrame = value;
			}
		}
	});

	Object.defineProperty(RollingTracer, "Steps", {
		get: function () { return RollingTracer.steps; },
		set: function (value) { RollingTracer.steps = value; }
	});
	...
	RollingTracer.resetCache = function () {
		...
		RollingTracer.spinSinStore = {};
		RollingTracer.spinCosStore = {};
		// RollingTracer.tracingDistanceSinStore = [];
	};
	...
	(function () {
		for (var i = 0; i <= (RollingTracer.period() * RollingTracer.TRACING_DISTANCE_MULT); i++) {
			...
			RollingTracer.tracingDistanceSinStore.push(Math.sin(rad));
			// RollingTracer.tracingDistanceCosStore.push(Math.cos(rad));
	...
	RollingTracer.prototype.updateTracingPoint = function() {
		...
		this.hyperAlive = this.spinAlive;
		this.rodAlive = this.tracingDistanceAlive;
	...
	RollingTracer.prototype.updateRod = RollingTracer.prototype.updateTracingPoint;
	RollingTracer.prototype.drawPoint = RollingTracer.prototype.drawTracingPoint;
